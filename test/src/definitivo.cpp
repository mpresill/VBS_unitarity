...
void Get_PT(TH1F* observable_l1, TH1F* observable_l2, TH1F* observable_j1, TH1F* observable_j2, 
     const TClonesArray* branchParticle, int npart, double w, double xminl, double xmaxl, double xminj, double xmaxj, int bins);
...
// and sign, e.g. "cW_m0p1" for cW = -0.1 for full analysis or "cW_int"/"cW_quad" for one-operator analysis.
  ...
  cout << "Parameters for reweighting are defined correctly." << endl;
  cout << "Luminosity: " << lum << " inverse pb" << endl;
  cout << "Position in input array is " << position << endl;
  cout << "~~~~~~~~~" << endl;
  ...
  else if   (strcmp(argv[1],"mll")        == 0){  xmax  = 560;
  						  xmin  = 0;

  }                           
  else if   (strcmp(argv[1],"PT")         == 0){  xmaxl = 900;
  						  xminl = 0;
  						  xmaxj = 1000;
  						  xminj = 0; 
  }                            						
...
//  double_t edges[NBINS + 1], edgesl[NBINSl + 1], edgesj[NBINSj + 1], edges1[NBINS1 + 1], edges2[NBINS2 + 1], edges12[NBINS12 + 1];
  ...
  TH1F *observable_j1=new TH1F(name_4j1,"",bins,xminj,xmaxj);
  TH1F *observable_j2=new TH1F(name_4j2,"",bins,xminj,xmaxj);
  ...
  cout << "Histograms for each observable have been set." << endl;
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  for(int i=0; i<treeReader->GetEntries(); i++){
    ...
    double w;
    double conv = 100000;
    if(strcmp(argv[2],"full")==0)          w = rwgt*ev_wgt*conv;
    else if(strcmp(argv[2],"single")==0)   w = ev_wgt*conv;  
    ...
    else if (strcmp(argv[1],"PT")==0)              	Get_PT(observable_l1, observable_l2, observable_j1, observable_j2, branchParticle, npart, w, xminl, xmaxl, xminj, xmaxj, bins);
    else if (strcmp(argv[1],"Eta")==0)             	Get_Eta(observable_l1, observable_l2, observable_j1, observable_j2, branchParticle, npart, w, xminl, xmaxl, xminj, xmaxj, bins);
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ HISTOGRAMS FOR COMBINE-FRIENDLY ROOT FILES ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  
  /* Single observable */
  char outfile[50];
  sprintf(outfile, "histos/%s/%s_%s.root",argv[1],argv[3],argv[2]);
  TFile* risultati=new TFile(outfile, "RECREATE");
  
  /* PT and Eta */
  char outfile_l1[50];
  sprintf(outfile_l1, "histos/%s_l1/%s_%s.root",argv[1],argv[3],argv[2]);
  TFile* risultati_l1=new TFile(outfile_l1, "RECREATE");
  char outfile_l2[50];
  sprintf(outfile_l2, "histos/%s_l2/%s_%s.root",argv[1],argv[3],argv[2]);
  TFile* risultati_l2=new TFile(outfile_l2, "RECREATE");
  char outfile_j1[50];
  sprintf(outfile_j1, "histos/%s_j1/%s_%s.root",argv[1],argv[3],argv[2]);
  TFile* risultati_j1=new TFile(outfile_j1, "RECREATE");
  char outfile_j2[50];
  sprintf(outfile_j2, "histos/%s_j2/%s_%s.root",argv[1],argv[3],argv[2]);
  TFile* risultati_j2=new TFile(outfile_j2, "RECREATE");
 
  /* Zeppenfeld */
  char outfile_1[50];
  sprintf(outfile_1, "histos/%s_1/%s_%s.root",argv[1],argv[3],argv[2]);
  TFile* risultati_1=new TFile(outfile_1, "RECREATE");
  char outfile_2[50];
  sprintf(outfile_2, "histos/%s_2/%s_%s.root",argv[1],argv[3],argv[2]);
  TFile* risultati_2=new TFile(outfile_2, "RECREATE");
  char outfile_12[50];
  sprintf(outfile_12, "histos/%s_12/%s_%s.root",argv[1],argv[3],argv[2]);
  TFile* risultati_12=new TFile(outfile_12, "RECREATE");

  cout << "No worries for errors, they're not true errors." << endl;
  cout << "~~~~~~~~~" << endl;
  cout << endl;
  ...
  if(strcmp(argv[1],"Eta")==0||strcmp(argv[1],"PT")==0){  
  		...
  		cout << argv[1] << "_l1 is ready for writing." << endl;
  		
  		risultati_l2  -> cd();
  		observable_l2 -> Write();
  		risultati_l2  -> Close();
  		
  		cout << argv[1] << "_l2 is ready for writing." << endl;
  		
  		risultati_j1  -> cd();
  		observable_j1 -> Write();
  		risultati_j1  -> Close();
  		
  		cout << argv[1] << "_j1 is ready for writing." << endl;
  		
  		risultati_j2  -> cd();
  		observable_j2 -> Write();
  		risultati_j2  -> Close();
  		
  		cout << argv[1] << "_j2 is ready for writing." << endl;
  }                
       		...
       		cout << argv[1] << "_1 is ready for writing." << endl;
       		
       		risultati_2   -> cd();
  		observable_2  -> Write();
  		risultati_2   -> Close();
  		
  		cout << argv[1] << "_2 is ready for writing." << endl;
  		
  		risultati_12  -> cd();
  		observable_12 -> Write();
  		risultati_12  -> Close();
  		
  		cout << argv[1] << "_12 is ready for writing." << endl;
  }   
  else{   	risultati     -> cd();
                observable    -> Write();
                risultati     -> Close();
                
                cout << argv[1] << " is ready for writing." << endl;
  }
...
void Get_Eta(TH1F* observable_l1, TH1F* observable_l2, TH1F* observable_j1, TH1F* observable_j2, 
  ...
  if	   (PTj[0]>PTj[1]){ 	int    nbins1 = observable_j1->GetNbinsX();
                      		...
                      		if       (Etaj[0]>xmaxj)  observable_j1->Fill(ovfl1,w);
                      		else if  (Etaj[0]<xminj)  observable_j1->Fill(undfl1,w);
	                        ...
	                        int    nbins2 = observable_j2->GetNbinsX();
                                ...
                                if       (Etaj[1]>xmaxj)  observable_j2->Fill(ovfl2,w);
                                else if  (Etaj[1]<xminj)  observable_j2->Fill(undfl2,w);
	                        else                      observable_j2->Fill(Etaj[1],w);                       
  }
  else if  (PTj[0]<PTj[1]){     int    nbins1 = observable_j1->GetNbinsX();
                                double ovfl1  = xmaxj - (xmaxj-xminj)/(nbins1+3);
                      		double undfl1 = xminj + (xmaxj-xminj)/(nbins1+3);
                      		if       (Etaj[1]>xmaxj)  observable_j1->Fill(ovfl1,w);
                      		else if  (Etaj[1]<xminj)  observable_j1->Fill(undfl1,w);
	              		else                      observable_j1->Fill(Etaj[1],w);      
	                        int    nbins2 = observable_j2->GetNbinsX();
                      		double ovfl2  = xmaxj - (xmaxj-xminj)/(nbins2+3);
                      		double undfl2 = xminj + (xmaxj-xminj)/(nbins2+3);
                      		if       (Etaj[0]>xmaxj)  observable_j2->Fill(ovfl2,w);
                      		else if  (Etaj[0]<xminj)  observable_j2->Fill(undfl2,w);
...
void Get_PT(TH1F* observable_l1, TH1F* observable_l2, TH1F* observable_j1, TH1F* observable_j2, 
  ...
  if		(PTj[0]>PTj[1]){  	int    nbins1 = observable_j1->GetNbinsX();
                      			double ovfl1  = xmaxj - xmaxj/(nbins1+3);
                      			if     (PTj[0]>xmaxj)  observable_j1->Fill(ovfl1,w);
	              			else                   observable_j1->Fill(PTj[0],w);      
	              			int    nbins2 = observable_j2->GetNbinsX();
                      			double ovfl2  = xmaxj - xmaxj/(nbins2+3);
                      			if     (PTj[1]>xmaxj)  observable_j2->Fill(ovfl2,w);
	              			else                   observable_j2->Fill(PTj[1],w);                       
  }
  else if	(PTj[0]<PTj[1]){  	int    nbins1 = observable_j1->GetNbinsX();
                      			double ovfl1  = xmaxj - xmaxj/(nbins1+3);
                      			if     (PTj[1]>xmaxj)  observable_j1->Fill(ovfl1,w);
	              			else                   observable_j1->Fill(PTj[1],w);      
	              			int    nbins2 = observable_j2->GetNbinsX();
                      			double ovfl2  = xmaxj - xmaxj/(nbins2+3);
                      			if     (PTj[0]>xmaxj)  observable_j2->Fill(ovfl2,w);
